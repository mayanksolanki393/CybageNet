...
<body>
	...
	<center>
		...
		<c:if test="${!empty param.review}">
			<jsp:useBean id="reviewDao" class="cyb.cybnet.dao.impl.ReviewDAOImpl"></jsp:useBean>
			<jsp:useBean id="review" class="cyb.cybnet.dto.Review"></jsp:useBean>
			<jsp:setProperty property="reviewBook" name="review" value="${param.bk_id}"/>
			<jsp:setProperty property="reviewReview" name="review" value="${param.review}"/>
			<jsp:setProperty property="reviewBy" name="review" value="${sessionScope.user.userId}"/>
			${reviewDao.addReview(review)}
			<c:redirect url="/ShowDescription.jsp?bookId=${param.bk_id}"></c:redirect>
		</c:if>
